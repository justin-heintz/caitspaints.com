...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'post-wrapper' ); ?>>
	...
	<header class="entry-header">
		<?php if ( ! is_single() && get_the_title() != '' ) : ?>
			<h1 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'matala' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h1>
		<?php else: ?>
		...
		<div class="post-format-icon"><a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'matala' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a></div>
			...
			<?php
				...
				<div class="gallery-wrap">
					...
					<p><strong><em><?php printf( _n( 'This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'matala' ),
										'href="' . get_permalink() . '" title="' . esc_attr( sprintf( __( 'Permalink to %s', 'matala' ), the_title_attribute( 'echo=0' ) ) ) . '" rel="bookmark"',
