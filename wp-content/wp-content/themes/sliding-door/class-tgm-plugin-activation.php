...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		 ...
		 * does the 'slidingdoor'_init action hook, and hooks in the interactions to init.
		 *
		 * @since 1.0.0
		 *
		...
		public function __construct() {
			...
			$this->strings = array(
				'page_title'                      => __( 'Install Required Plugins', 'slidingdoor' ),
				'menu_title'                      => __( 'Install Plugins', 'slidingdoor' ),
				'installing'                      => __( 'Installing Plugin: %s', 'slidingdoor' ),
				'oops'                            => __( 'Something went wrong.', 'slidingdoor' ),
				...
				'return'                          => __( 'Return to Required Plugins Installer', 'slidingdoor' ),
				'plugin_activated'                => __( 'Plugin activated successfully.', 'slidingdoor' ),
				'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'slidingdoor' ),
			);

			/** Annouce that the class is ready, and pass the object (for advanced use) */
			do_action_ref_array( 'slidingdoor_init', array( &$this ) );
		...
		public function init() {
			...
			do_action( 'slidingdoor_register' );
		 ...
		 *
		 * We get the links in the admin notices to point to the 'slidingdoor' page, rather
		 ...
		 * @return null Returns early if not the 'slidingdoor' page.
		 */
		...
		public function thickbox() {
			...
			if ( ! get_user_meta( get_current_user_id(), 'slidingdoor_dismissed_notice', true ) )
				add_thickbox();
		...
		public function install_plugins_page() {
			...
			$plugin_table = new slidingdoor_List_Table;

			/** Return early if processing a plugin installation action */
			if ( isset( $_POST[sanitize_key( 'action' )] ) && 'slidingdoor-bulk-install' == $_POST[sanitize_key( 'action' )] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() )
				return;

			?>
			<div class="'slidingdoor' wrap">

				<?php screen_icon( apply_filters( 'slidingdoor_default_screen_icon', 'themes' ) ); ?>
				<h2><?php echo esc_html( get_admin_page_title() ); ?></h2>
				<?php $plugin_table->prepare_items(); ?>

				<?php if ( isset( $this->message ) ) _e( wp_kses_post( $this->message ), 'slidingdoor' ); ?>

				<form id="'slidingdoor'-plugins" action="" method="post">
            		<input type="hidden" name="'slidingdoor'-page" value="<?php echo $this->menu; ?>" />
		...
		protected function do_plugin_install() {
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'slidingdoor-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'slidingdoor-install' )] ) ) {
				check_admin_referer( 'slidingdoor-install' );
				...
				$url = wp_nonce_url(
					add_query_arg(
						array(
							...
							'slidingdoor-install' => 'install-plugin',
						),
				...
				$fields = array( sanitize_key( 'slidingdoor-install' ) ); // Extra fields to pass to WP_Filesystem
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( 'Return to Required Plugins Installer', 'slidingdoor' ) . '</a></p>';
				...
				foreach ( $this->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( $this->strings['return'], 'slidingdoor' ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'slidingdoor' ) . '">' . __( 'Return to the Dashboard', 'slidingdoor' ) . '</a>' ) . '</p>';
			...
			elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'slidingdoor-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'slidingdoor-activate' )] ) ) {
				check_admin_referer( 'slidingdoor-activate', 'slidingdoor-activate-nonce' );
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
						$msg = sprintf( __( 'The following plugin was activated successfully: %s.', 'slidingdoor' ), '<strong>' . $plugin['name'] . '</strong>' );
			...
			if ( ! get_user_meta( get_current_user_id(), 'slidingdoor_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'slidingdoor' ), $imploded, $count ) . '</p>'; // All messages now stored
					}

					/** Setup variables to determine if action links are needed */
					$show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'slidingdoor' ) . '</a>' : '';
					$show_activate_link = $activate_link ? '<a href="' . admin_url( 'plugins.php' ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'slidingdoor' ) . '</a>'  : '';

					/** Define all of the action links */
					$action_links = apply_filters(
						'slidingdoor_notice_action_links',
						array(
							'install'  => ( current_user_can( 'install_plugins' ) ) ? $show_install_link : '',
							'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'slidingdoor-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'Dismiss this notice', 'slidingdoor' ) . '</a>',
						)
					);
					...
					/** Register the nag messages and prepare them to be processed */
               		if ( isset( $this->strings['nag_type'] ) )
						add_settings_error( 'slidingdoor', 'slidingdoor', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ), 'updated' ) );
					else
						add_settings_error( 'slidingdoor', 'slidingdoor', $rendered, 'updated' );
				}
			}

			/** Admin options pages already output settings_errors, so this is to avoid duplication */
			if ( 'options-general' !== $current_screen->parent_base )
				settings_errors( 'slidingdoor' );
		...
		public function dismiss() {
			...
			if ( isset( $_GET[sanitize_key( 'slidingdoor-dismiss' )] ) )
				update_user_meta( get_current_user_id(), 'slidingdoor_dismissed_notice', 1 );
		...
		public function actions( $install_actions ) {
			...
			/** Remove action links on the 'slidingdoor' install page */
			if ( $this->is_slidingdoor_page() )
				return false;
		...
		/**
		 * Determine if we're on the 'slidingdoor' Install page.
		 *
		 ...
		 *
		 * @global object $current_screen
		 * @return boolean True when on the 'slidingdoor' page, false otherwise.
		 */
		protected function is_slidingdoor_page() {
		...
		public function update_dismiss() {
			...
			delete_user_meta( get_current_user_id(), 'slidingdoor_dismissed_notice' );
...
if ( ! function_exists( 'slidingdoor' ) ) {
	/**
	 ...
	 */
	function slidingdoor( $plugins, $config = array() ) {
...
if ( ! class_exists( 'slidingdoor_List_Table' ) ) {
	/**
	 ...
	 * @author Gary Jones <gamajo@gamajo.com>
	 */
	class slidingdoor_List_Table extends WP_List_Table {

		/**
		 * References parent constructor and sets defaults for class.
		 *
		 * The constructor also grabs a copy of $instance from the 'slidingdoor' class
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', 'slidingdoor' );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', 'slidingdoor' );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', 'slidingdoor' );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', 'slidingdoor' );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', 'slidingdoor' ) : __( 'Recommended', 'slidingdoor' );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'slidingdoor' ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'slidingdoor' ) );
					...
					'activate' => sprintf(
						...
						add_query_arg(
							array(
								...
								'plugin_name'          => $item['sanitized_plugin'],
								'plugin_source'        => $item['url'],
								'slidingdoor-activate'       => 'activate-plugin',
								'slidingdoor-activate-nonce' => wp_create_nonce( 'slidingdoor-activate' ),
							),
							admin_url( TGM_Plugin_Activation::$instance->parent_url_slug )
						),
						$item['sanitized_plugin']
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'slidingdoor' ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'slidingdoor' ),
				'source' => __( 'Source', 'slidingdoor' ),
				'type'   => __( 'Type', 'slidingdoor' ),
				'status' => __( 'Status', 'slidingdoor' )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'slidingdoor-bulk-install'  => __( 'Install', 'slidingdoor' ),
				'slidingdoor-bulk-activate' => __( 'Activate', 'slidingdoor' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'slidingdoor-bulk-install' === $this->current_action() ) {
				...
				$url = wp_nonce_url(
					add_query_arg(
						array(
							'page' 			=> TGM_Plugin_Activation::$instance->menu,
							'slidingdoor-action' 	=> 'install-selected',
				...
				/** Loop through each plugin to install and try to grab information from WordPress API, if not create 'slidingdoor-empty' scalar */
				foreach ( $plugin_installs as $plugin ) {
					$api[$c] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$c] = 'slidingdoor-empty';
					$c++;
				}
				...
				echo '<div class="slidingdoor wrap">';
					screen_icon( apply_filters( 'slidingdoor_default_screen_icon', 'themes' ) );
			...
			if ( 'slidingdoor-bulk-activate' === $this->current_action() ) {
				check_admin_referer( 'bulk-' . $this->_args['plural'] );
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'slidingdoor' ), $imploded );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			/**
	 		 ...
	 		 * set to true for the 'slidingdoor' class.
	 		 *
	 		 * @since 2.2.0
	 		 *
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', 'slidingdoor' );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'slidingdoor' );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'slidingdoor' );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'slidingdoor' );
				$this->strings['process_failed']      = __( 'Plugin install failed.', 'slidingdoor' );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', 'slidingdoor' );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'slidingdoor' );
				$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'slidingdoor' );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'slidingdoor' );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'slidingdoor' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'slidingdoor' ) . '</span><span class="hidden">' . __( 'Hide Details', 'slidingdoor' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'slidingdoor' );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'slidingdoor' );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'slidingdoor' );
					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'slidingdoor' );
					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'slidingdoor' );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'slidingdoor' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'slidingdoor' ) . '</span><span class="hidden">' . __( 'Hide Details', 'slidingdoor' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'slidingdoor' );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'slidingdoor' );
				}
			...
			public function bulk_footer() {
				...
				foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu, admin_url( TGM_Plugin_Activation::$instance->parent_url_slug ) ) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . __( TGM_Plugin_Activation::$instance->strings['return'], 'slidingdoor' ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'slidingdoor' ) . '">' . __( 'Return to the Dashboard', 'slidingdoor' ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
