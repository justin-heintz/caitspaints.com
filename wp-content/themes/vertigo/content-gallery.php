...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="container">
		...
		<header class="entry-header hitchcock">
			...
			<?php if ( ! is_single() ) : ?>
				<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'vertigo' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a>
			<?php else : ?>
				<?php the_title(); ?>
			<?php endif; ?>
			</h1>
				...
				<?php if ( post_password_required() ) : ?>
					...
					<?php else : ?>
						...
						<p><em><?php printf( _n( 'This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'vertigo' ),
								'href="' . get_permalink() . '" title="' . esc_attr( sprintf( __( 'Permalink to %s', 'vertigo' ), the_title_attribute( 'echo=0' ) ) ) . '" rel="bookmark"',
