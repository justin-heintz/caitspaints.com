...
function forever_home_posts( $query = false ) {
	...
	if ( ! is_home() || ! is_a( $query, 'WP_Query' ) || ! $query->is_main_query() || ( ! forever_featured_posts() && ! forever_recent_four_posts() ) )
		return $query;

	// Exclude featured posts from the main query
	$query->query_vars['post__not_in'] = array_merge( (array) forever_featured_posts(), (array) forever_recent_four_posts() );
