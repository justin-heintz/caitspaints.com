			...
			<div id="sticky-posts">
				...
				<?php while ( $the_query->have_posts() ) : $the_query->the_post(); $counter++; ?>
					<div class="sticky-post fl <?php if ( $counter == 3 ) { echo 'last'; } ?>">
						<h4><a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'woothemes' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h4>
						<?php the_excerpt(); ?>
						<a class="more" href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'woothemes' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php _e( 'Continue Reading', 'woothemes' ); ?></a>
					</div><!-- /.sticky-post -->
