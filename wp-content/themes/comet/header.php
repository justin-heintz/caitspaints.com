...
<?php do_action( 'before' ); ?>
	...
	<div id="header">
		...
		if ( ! empty( $header_image ) )
			echo '<a id="header-image" href="' . esc_url( home_url() ) . '"><img src="' . esc_url( $header_image ) . '" alt="" /></a>';

		echo '<h1 id="site-title"><a href="' . esc_url( home_url() ) . '">' . get_option( 'blogname' ) . '</a></h1>';

		$tagline = get_bloginfo( 'description' );
		if ( ! empty( $tagline ) )
			echo '<a id="site-description" href="' . esc_url( home_url() ) . '">' . $tagline . '</a>';
		?>
	</div><!-- /header -->
