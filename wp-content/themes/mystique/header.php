					...
					<?php if ( has_post_thumbnail() ) :
						...
						if ( $image >= HEADER_IMAGE_WIDTH ) : ?>
							<div class="featured-post featured-tiptrigger">
								<h2 class="showcase-heading"><?php echo( esc_attr( $options['featured_post_label'] ) ); ?> <a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'mystique' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
								<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'mystique' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"> <?php the_post_thumbnail( 'large-feature' ); ?></a>
								<?php
